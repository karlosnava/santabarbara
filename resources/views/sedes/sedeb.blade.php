<div class="w-full">
	...
	<div class="grid grid-cols-5">
		<div class="col-span-5 sm:col-span-2">
			<img src="{{ Storage::url($locationB->image) }}" class="w-full object-cover object-center shadow-lg rounded-lg" alt="{{ $locationB->name }}">
		</div>
		<div class="col-span-5 px-2 mt-3 sm:col-span-3 sm:mt-0 sm:px-4">
			<div class="font-bold text-xl mb-2">{{ $locationB->name }} <span class="text-gray-700 text-sm"> | {{ $locationB->opens }} - {{ $locationB->closes }}</span></div>
			<p class="text-gray-700 text-sm">{{ $locationB->description }}</p>
			<hr class="my-3">
			<div class="text-gray-700 text-sm text-right"><i class="fas fa-map-marker-alt"></i> {{ $locationB->direction }} <span class="mx-2">|</span> <a href="tel:{{ $locationB->phone }}"><i class="fas fa-phone rotate-90"></i> {{ $locationB->phone }}</a></div>
		</div>
	</div>
    ...
    @if(count($locationB->posts->where('status', 'active')) > 0)
	    <div class="grid grid-cols-9 gap-5">
		    @foreach($locationB->posts->where('status', 'active') as $post)
